      ...
      <div class="actions">
        <div>
          <div
            ...
            <button
              class="white"
              @click="remove">Remove Recipients</button>
          </div>
        </div>
        <DataOptions>
          <HeaderFilters
...
export default {
  ...
  watch: {
    selectedTag() {
      this.fetchListRecipients().then((fetchData) => {
  ...
  methods: {
    ...
    ...mapActions(campaignsNamespace, ['fetchCampaign', 'fetchListRecipients', 'removeRecipient']),
    hideHeaderFilters() {
    ...
    pageContacts(next) {
      ...
      this.fetchListRecipients({ ...pageReq })
        .then((fetchData) => {
    ...
    search() {
      this.fetchListRecipients({ before: null, after: null, query: this.query })
        .then((fetchData) => {
          this.setContacts({ ...fetchData, index: 0 });
        });
    },
    remove() {
      ...
      this.selectedContacts.forEach((id) => {
        const recipientId = this.contacts.contacts
          .find((contact) => id === contact.id).recipient_id;
        promises.push(this.removeRecipient(recipientId));
      });
    ...
    initRecipients() {
      ...
      this.fetchCampaign(this.campaignId).then((campaign) => {
        ...
        axios
          .all([this.fetchListRecipients(), this.fetchTags()])
          .then(([fetchData, { tags }]) => {
            this.SET_TAGS(tags);
            this.setContacts({ ...fetchData, index: 0 });
